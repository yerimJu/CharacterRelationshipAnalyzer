...
# <<<추가한 항목 : kkma 라이브러리로 명사만 골라내기
word_list = []

# kkma = Kkma()
# word_dic = kkma.pos(sentences)

# hannanum = Hannanum()
# word_dic = hannanum.pos(sentences)

# twitter = Twitter()
# word_dic = twitter.pos(sentences)

komoran = Komoran()
word_dic = komoran.pos(sentences)

print('finding pos func done')

# str.startswitch 함수가 동작하지 않아서 find 함수를 씀
for w in word_dic:
    if w[1].find('N') == 0:
        word_list.append(w[0])
print('명사 집합 상위 10개 : ',word_list[:10])
freq_num = int(len(word_list) / 120)

# 추가한 항목>>> : 일정 빈도수 이상의 word만 골라냄
word_vocab = nltk.Text(word_list).vocab()
# print(word_vocab['백설공주'])
word_list.clear()
